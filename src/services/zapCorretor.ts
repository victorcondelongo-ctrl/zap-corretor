...
export async function getCurrentProfile(): Promise<ZapProfile> {
  try {
    ...
    const typedProfile: ZapProfile = {
      ...
      tenant_name: (profile.tenants as { name: string }[] | null)?.[0]?.name || null, // CORRECTED
...
export const adminTenantService = {
  ...
  async getSettings(): Promise<ZapTenantSettings> {
    ...
    // Extract tenant name
    const tenantName = (data.tenants as { name: string }[] | null)?.[0]?.name || profile.tenant_name || 'Corretora'; // CORRECTED
